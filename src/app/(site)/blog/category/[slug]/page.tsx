...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return buildPageMetadata({
    ...
    description: `Browse blog articles and publications in the ${slug} category.`,
  });
}
...
export default async function BlogCategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const breadcrumbSchema = buildBreadcrumbSchema([
    { name: "Home", url: "/" },
    { name: "Blog", url: "/blog" },
    { name: slug, url: `/blog/category/${slug}` },
  ]);

  return (
    <section>
      <JsonLdScript id={`insight-category-breadcrumb-${slug}`} data={breadcrumbSchema} />
      <Breadcrumbs items={[{ label: "Home", href: "/" }, { label: "Blog", href: "/blog" }, { label: slug }]} />
      <h1 className="text-4xl font-semibold">Category: {slug}</h1>
      <div className="mt-8 grid gap-5 md:grid-cols-3">
        {insights.data.map((item) => (
          <Link key={item.id} href={`/blog/${item.slug}`} className="rounded-2xl border border-[var(--glass-border)] bg-surface p-6">
