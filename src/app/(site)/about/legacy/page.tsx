    ...
    <div className="flex flex-col min-h-screen bg-bg">
      ...
      <section className="mx-auto max-w-5xl px-6 py-20 md:py-32 w-full">
        <div className="text-center mb-20">
          <h2 className="text-3xl md:text-4xl font-bold text-ink mb-6">Our Journey</h2>
        ...
        <div className="relative border-l-2 border-[#18395f]/20 ml-4 md:ml-1/2">
          {milestones.map((item, i) => (
            <div key={i} className="mb-12 relative pl-8 md:pl-0">
              <div className="absolute w-5 h-5 bg-accent-secondary rounded-full -left-[11px] top-1 border-4 border-stone-50 md:left-1/2 md:-ml-[10px]" />
              
              <div className={`md:w-1/2 ${i % 2 === 0 ? 'md:pr-12 md:text-right' : 'md:pl-12 md:ml-auto'}`}>
                <span className="text-accent-secondary font-bold text-xl mb-2 block">{item.year}</span>
                <h3 className="text-2xl font-bold text-ink mb-3">{item.title}</h3>
                <p className="text-muted leading-relaxed bg-surface p-6 rounded-xl shadow-sm border border-[var(--glass-border)]">
                  {item.desc}
                </p>
              </div>
            </div>
