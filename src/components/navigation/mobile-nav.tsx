  ...
  return (
    <>
      <button
        ...
        className="xl:hidden inline-flex h-10 w-10 items-center justify-center rounded-md border border-[var(--glass-border)] bg-surface-raised text-ink transition-colors hover:text-accent"
      ...
      >
        ...
        <aside
          ...
          className={`absolute right-0 top-0 h-full w-[86%] max-w-sm border-l border-[var(--glass-border)] bg-surface shadow-2xl transition-transform duration-300 ease-out ${
            isOpen ? "translate-x-0" : "translate-x-full"
          }`}
        >
          <div className="flex items-center justify-between border-b border-[var(--glass-border)] px-5 py-4">
            <p className="text-sm font-bold uppercase tracking-[0.1em] text-ink">Menu</p>
            <button
              type="button"
              aria-label="Close menu"
              className="inline-flex h-9 w-9 items-center justify-center rounded-md border border-[var(--glass-border)] text-ink hover:text-accent"
              onClick={() => setIsOpen(false)}
            >
          ...
          <nav aria-label="Mobile" className="h-[calc(100%-65px)] overflow-y-auto px-4 py-4">
            <div className="flex flex-col gap-2">
              {navGroups.map(({ group, items }) => {
                ...
                return (
                  <div key={group} className="rounded-md border border-[var(--glass-border)] bg-surface-raised/60">
                    <button
                      type="button"
